...
function FlagCard({ group, onSelect }: { group: FlagGroup; onSelect: (e: Employee) => void }) {
  ...
  return (
    <div className={`rounded-xl border p-3 flex flex-col gap-2 ${
      isEmpty || group.pending ? "bg-gray-50 border-gray-200" : `${colors.bg} ${colors.border}`
    }`}>
      {/* Header */}
      ...
      {group.pending ? (
        ...
        <>
          <ul className="space-y-1">
            {shown.map((item) => (
              <li key={item.employee.employeeId} className="flex items-center justify-between gap-2">
                <button
                  onClick={() => onSelect(item.employee)}
                  className="text-xs text-gray-700 hover:text-indigo-600 hover:underline truncate font-medium transition-colors text-left"
                >
                  {item.employee.name}
                </button>
    ...
    <>
      <div className="bg-white rounded-2xl shadow-[0_4px_24px_rgba(0,0,0,0.06)] p-4">
        ...
        <div className="grid grid-cols-2 gap-2.5">
          {groups.map((g) => (
            <FlagCard key={g.id} group={g} onSelect={setSelected} />
          ))}
        </div>
      </div>
